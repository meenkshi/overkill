...
class DetailsDisclosure extends HTMLElement {
   ...
   */
  addContentHeight() {
    if(this.hasAttribute('data-readmore')){
      this.panel.style.height = `3rem`;
    }else{
...
const FilterContainer = class extends HTMLElement {
  constructor() {
    ...
    if (this.dataset.ajaxFiltering === 'true') {
      // ajax load on link pointerdown (touch devices)
      theme.addDelegateEventListener(this.section, 'pointerdown', '.link-dropdown__link, .filter-group__applied-item, .filter-group__clear-link, .pagination a', (evt, delEl) => {
        evt.preventDefault();
        if (window.matchMedia('(pointer: none)').matches) {
          this.ajaxLoadUrl(delEl.href);
        }
      });
      // ajax load on link click (non-touch devices)
  ...
  connectedCallback() {
    ...
    this.delegatedToggleFiltersCallback = theme.addDelegateEventListener(this.section, 'click', '[data-toggle-filters]', (evt) => {
      evt.preventDefault();
      document.querySelector('body').classList.toggle('reveal-mobile-filter');
      ...
      if (evt.target.closest('.toggle-btn').classList.contains('filter-init')) {
        evt.target.closest('.toggle-btn').classList.remove('filter-init');
        if (window.matchMedia('(max-width: 767px)').matches) {
          this.section.querySelector('.filters').classList.add('filter-init');
          this.section.querySelectorAll('toggle-target:not([data-toggle-target=".filter-group--sort"])').forEach((el) => {
            el.classList.add('toggle-target-toggler--is-hidden');
            el.nextElementSibling.classList.add('toggle-target--hidden');
          });
          setTimeout(() => {
            this.section.querySelector('.filters').classList.remove('filter-init');
          }, 300);
        }
      }
    });
...
const GalleryViewer = class extends HTMLElement {
  ...
  alterCurrentTransformZoomBy(delta) {
    ...
    this.currentTransform.zoom = Math.min(this.currentTransform.zoom, 2.0);
  ...
  updateImagePosition() {
    ...
    this.currentZoomImage.style.transform = `translate3d(${this.currentTransform.panX * (this.currentMaxZoom + 2.5)}px, ${this.currentTransform.panY * (this.currentMaxZoom + 2.5)}px, 0) scale(${this.currentTransform.zoom})`;
  }
  ...
  onZoomContainerClick(evt) {
    evt.preventDefault();
    this.currentMaxZoom++
    //console.log('Before',this.currentMaxZoom);
    if (this.currentTransform.zoom === 2.0) {
      //console.log('Über 2');
      this.currentTransform.zoom = 0;
      this.alterCurrentTransformZoomBy(0);
    } else {
      this.currentTransform.zoom = 1 * this.currentMaxZoom;
      this.alterCurrentTransformZoomBy(0);
      this.panZoomImageFromCoordinate(evt.clientX, evt.clientY);
    }
    if(this.currentMaxZoom >= 3){
      this.currentMaxZoom = 0;
    }
    console.log(this.currentTransform.zoom);
...
const ProductForm = class extends HTMLElement {
  ...
  async handleSubmit(evt) {
    ...
    try {
      ...
      document.querySelector("page-header").refreshHeader();
...
if (!window.customElements.get('quantity-wrapper')) {
  class QuantityWrapper extends HTMLElement {
    ...
    handleClick(evt) {
      ...
      if (btn) {
        ...
        /*Hasaan@hasaanlatiflatori: Added Max Attribute Globally*/
        ...
        if(input.hasAttribute('min')){
          input.value = Math.max(0, parseInt(input.value, 10) + change);
  ...
  window.customElements.define('quantity-wrapper', QuantityWrapper);
  if (!window.customElements.get('quantity-wrapper-spray')) {
    class SprayWrapper extends QuantityWrapper {
      connectedCallback() {
        super.connectedCallback(); 

      }
    }
    window.customElements.define('quantity-wrapper-spray', SprayWrapper);
}
...
if (theme.settings.quickbuyStyle !== 'off') {
  theme.quickbuy = {
    ...
    setBlockHeights: (block) => {
      ...
      list.setAttribute('data-quickbuy-shown', true);
      if (!list) return;

      //Hasaan@Latori: Adding Max Height To collection cards, to not collapse with grid auto rows css
      const collectionCards = [...list.querySelectorAll('.collection-card')];
      collectionCards.forEach((el) => {
        el.style.setProperty('--qb-block-height', `${el.clientHeight}px`);
      ...
      const blocksInThisRow = [...list.querySelectorAll('.product-block , .collection-card')].filter((el) => el.offsetTop === blockTop);
      ...
      blocksInThisRow.forEach((el) => {
        ...
        el.querySelector('.block-inner').style.setProperty('--qb-block-height', `${tallest}px`);
          ...
          if(el.classList.contains('collection-card')){
            //Hasaan@Latori: Im adding a max height for the collection cards also, in order to not expand when quick buy is activated.
          console.log(el);
          el.style.setProperty('--qb-block-height', `${tallest}px`);
            }
          }
      });
    ...
    onCloseClick: (evt) => {
      ...
      const list = evt.target.closest('.product-grid');
      list.setAttribute('data-quickbuy-shown', false);
    }
  };
...
/**
 * Cookiebot related functionality
 */
document.addEventListener('DOMContentLoaded', () => {
  ...
  footerEl.addEventListener('click', (event) => {
    if (event.target.getAttribute('href') === '#cookieconsent') {
      openWidget();
    }
  });
});